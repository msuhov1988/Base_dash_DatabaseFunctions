...
from TabBase.Setups.SumObject import UNITED_CHECK_LIST
from TabBase.Setups.SetObject import DISCRETE_CHECK_LIST
...
def layout_create(drop_div, drop_reg, drop_city, drop_tt, end_date, beg_date):
                    ...
                    html.Div(className='base_row_reference', children=[
                        ...
                        dbc.Row([
                            ...
                            dbc.Col(
                                dcc.Dropdown(
                                    id="base_divisions",
                                    options=[{"label": v, "value": k} for k, v in drop_div.items()],
                                    value=[],
                                    placeholder="Итого",
                            ...
                            dbc.Col(
                                dcc.Dropdown(
                                    id="base_regions",
                                    options=[{"label": v, "value": k} for k, v in drop_reg.items()],
                                    value=[],
                                    placeholder="Итого",
                                    multi=True), width=2
                            ),
                            dbc.Col(
                                dcc.Dropdown(
                                    id="base_cities",
                                    options=[{"label": v, "value": k} for k, v in drop_city.items()],
                                    value=[],
                                    placeholder="Итого",
                                    multi=True), width=2
                            ),
                            dbc.Col(
                                dcc.Dropdown(
                                    id="base_shops",
                                    options=[{"label": v, "value": k} for k, v in drop_tt.items()],
                                    value=[],
                                    placeholder="Итого",
                                    multi=True), width=3
                            )
                    ...
                    dbc.Row([
                        ...
                        dbc.Col(
                            html.Div(className="types_for_choose", children=[
                                html.Div(className="base_body_carts_checklist", children=[
                                    dcc.Checklist(
                                        id="base_carts_checklist",
                                        options=[{'label': k, 'value': v} for k, v in DISCRETE_CHECK_LIST.items()],
                                        value=[],
                                        labelStyle={'display': 'block'})
                                ]),
                    ...
                    html.Div(className="base_row_reference", children=[
                        dbc.Row([
                            ...
                            dbc.Col(
                                html.Div(className="base_table_checklist", children=[
                                    ...
                                    dcc.Checklist(
                                        ...
                                        options=[{'label': v, 'value': k} for k, v in Bs.TABLE_SLICES.items()],
                                        value=[list(Bs.TABLE_SLICES.keys())[0]],
                                        labelStyle={'display': 'block'})
                                ]), width=2
                            ),
                            dbc.Col([
                            ...
                            dbc.Col([
                                ...
                                html.Div(className="base_table_checklist", children=[
                                    html.Div(className="base_date_range", children=[
                                        html.Label("Показатели по картам", className="label"),
                                        dcc.Dropdown(
                                            id="base_table_carts_references",
                                            options=[{"label": k, "value": v} for k, v in DISCRETE_CHECK_LIST.items()],
                                            value=[],
                                            multi=True)
                                    ])
